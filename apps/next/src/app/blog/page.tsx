...
import { ArticleContent, BlogContent, RecommendedQuestions } from "@/constants/BlogData";
...
const page = (props: Props) => {
  return (
    <div className="flex ">
    <div className="left_panel  w-[60%]">
      <div className="flex flex-col gap-6">
      {BlogContent.map((blog, idx)=>{
        return (
          <BlogCard title={blog.title} description={blog.description} img={blog.img} key={idx}/>
        );
      })}
      </div>
      
    </div>
    <div className="right_panel w-[40%]">
      ...
      <div className="flex flex-col gap-4 ml-4 mr-4">
        {RecommendedQuestions.map((ques,ques_idx)=>{
          return(
            <BlogRecommendedQuestions question={ques.question} key={ques_idx} />
          );
        })}
      </div>
      ...
      {ArticleContent.map((article, article_idx)=>{
        return (
          <ArticleCard title={article.title} description={article.description} date ={article.date} img={article.img} key={article_idx}/>
        );
      })}
      </div>
    </div>
